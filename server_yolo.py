    ...
    def do_GET(self):
        if self.path == '/':
            ...
            while True:
                ...
                #image = asarray(frame)
                #image = cv2.resize(frame, None, fx=1.0, fy=1.0, interpolation=cv2.INTER_AREA)
                image = cv2.imdecode(np.fromstring(frame, np.uint8), 1)
                ...
                #cv2.waitKey(1)
                retval, bufferImg = cv2.imencode('.jpg', image)
                response = bufferImg.tobytes()
